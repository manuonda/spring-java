...
import com.springboot.webflux.handlers.ProductoHandler;
...
public class RouterFunctionConfig {
	...
	public RouterFunction<ServerResponse> routes(ProductoHandler handler) {
		...
		return  route(GET("/api/v2/productos").or(GET("/api/v3/productos")), handler::listar)
				.andRoute(GET("/api/v2/productos/ver/{id}"), handler::ver)
				...
				.andRoute(POST("/api/v2/productos/uploadFile/{id}"), handler::uploadFile)
				.andRoute(POST("/api/v2/productos/crear2"), handler::crear2);
	}
}
